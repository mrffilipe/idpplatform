...
export class HttpClient {
  ...
  private async toApiError(status: number, text: string): Promise<IdPApiError> {
    try {
      const problem = JSON.parse(text) as ProblemDetails
      return new IdPApiError(problem.detail ?? problem.title ?? text, status, problem, text)
    } catch {
      return new IdPApiError(text || `HTTP ${status}`, status, undefined, text)
    }
  }
}
